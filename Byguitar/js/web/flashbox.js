...
function onbar(obj,obj_bar){	
	...
	{			
		...
		$(obj_bar).find('li').hover(function(){	
			...
			//$(this).addClass(this_on).siblings().removeClass(this_on);							  
			//$(obj).find('.switch_item').eq($(obj_bar).find('li').index(this)).fadeIn().siblings().hide();								 
...
function Slide(obj,index,obj_bar){
	...
	listArr.eq(index).fadeIn(200,function(){listArr.eq(prev).fadeOut(300);});
	break;
	case 'fadeIn':	
	listArr.eq(prev).fadeOut(200,function(){listArr.eq(index).fadeIn(300);});
	break;
	case 'slideUp':
